...
- **1Password Credentials** — Secrets stored in 1Password, referenced via `op://` URIs. No plaintext secrets on disk.
...
```bash
npm install
cp .env.example .env   # Fill in your values (op:// refs or plaintext)
...
All configuration lives in `.env` (generated by `deploy.sh`). Secrets are stored in 1Password and referenced via `op://` URIs — no plaintext secrets on disk. At runtime, `op run --env-file=.env` resolves references in-memory before passing them to docker-compose.
...
Secrets are managed via 1Password with `op://` references — **no plaintext secrets are ever written to disk**:

- **Deploy**: `deploy.sh` creates 1Password items and writes `op://` references to `.env`
- **Runtime**: `op run --env-file=.env -- docker compose up` resolves secrets in-memory
- **Rotate**: `./scripts/update.sh --reconfigure` updates the value in 1Password. Since `.env` contains references, no file changes are needed — the new value is picked up on next service restart
- **Vault**: All secrets live in your configured 1Password vault. Manage them via the 1Password app or CLI
